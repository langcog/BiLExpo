...
data_item <- bilingual_item_data_clean |> 
  ...
  #filter(is.element(item_id, sample(itids, 200))) |>
...
# Add the spline basis to your data
...
```{r, eval=F}
rstan_options(auto_write = TRUE) # 1000 transitions using 10 leapfrog steps per transition would take 337.84 seconds.
options(mc.cores = parallel::detectCores())

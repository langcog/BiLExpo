...
# try fitting only Swadesh-CDI items (for compute practicality)
swad_orig <- read_csv("data/Swadesh_items_from_4_methods.csv") |> 
  filter(method == "Syntactic") |> pull(uni_lemma)
# should also add optional items: 
swad_extension <- c("try", "in", "out", "under", "a lot", "all", "many", 
                    "not", "glue (object)", "toy (object)")
swad_items <- c(swad_orig, swad_extension)

bilingual_swad_item_data <- bilingual_item_data_clean |> filter(uni_lemma %in% swad_items)
```
...
stan_data <- prepare_stan_data(bilingual_swad_item_data, 
                               original_dataset_name = "Marchman Dallas Bilingual")
```
...
saveRDS(fit_linear, file="models/model2_SCDI_fit.rds")
rm(fit_linear)
```
...
fit_quad <- stan(file = "models/model5-quadratic-exposure.stan",
            ...
            iter = 4000, warmup = 1000, chains = 4, seed = 123)
saveRDS(fit_quad, file="models/model5_SCDI_fit.rds")
rm(fit_quad)
...
fit_cubic <- stan(file = "models/model6-cubic-exposure.stan",
            ...
            control = list(max_treedepth=12),
            iter = 4000, warmup = 1000, chains = 4, seed = 123)
saveRDS(fit_cubic, file="models/model6_SCDI_fit.rds")
rm(fit_cubic)
```
...
exposure_spline <- bs(stan_data$exposure, df = 4)

stan_data_spline <- stan_data |>
  within({
...
fit <- stan(file = "models/model4-exposure-spline.stan",
            data = stan_data_spline,
            model_name = "exposure-spline",
            control = list(max_treedepth=15),
            iter = 4000, warmup = 1000, chains = 4, seed = 123) # thin = 2
saveRDS(fit, file="models/model4-exposure-spline_SCDI_fit.rds")
```
...
fit_stan_by_lexical_category(
  ...
  original_dataset_name = "Marchman Dallas Bilingual",
  seed = 123,
  vb_args = list(iter = 100000, output_samples = 1000, tol_rel_obj = 0.0002)
)
```
...
fit_stan_by_lexical_category(
  data = bilingual_item_data_clean,
  model_file = "models/model3-age-spline.stan",
  output_dir = "models",
  method = "vb",
  original_dataset_name = "Marchman Dallas Bilingual",
  vb_args = list(iter = 100000, output_samples = 1000, tol_rel_obj = 0.0002)
...
```{r}
# Check the results
fit <- readRDS("models/model4-exposure-spline_CDI_fit.rds")
#plot(fit)
...
ggplot(spline_df, aes(x = exposure_proportion)) +
  geom_line(aes(y = predicted_spline), color = "blue", linewidth = 1) +
  geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.2, fill = "blue") +
...
```{r}
# Check the results
fit_quad <- readRDS("models/model5_SCDI_fit.rds")
plot(fit_quad, pars=c("beta_age", "beta_exposure", "beta_exposure2", "beta_age_exposure")) # alpha
```
...
```{r}
# Check the results
fit_cubic <- readRDS("models/model6_SCDI_fit.rds")
...
# scdi_models = c("model2_SCDI_fit.rds","model5_SCDI_fit.rds","model6_SCDI_fit.rds","model4-exposure-spline_SCDI_fit.rds")

fit_linear <- readRDS("models/model2_SCDI_fit.rds")
...
fit_spline <- readRDS("models/model4-exposure-spline_SCDI_fit.rds")
log_lik_spline  <- extract_log_lik(fit_spline, merge_chains = FALSE)
rm(fit_spline)
...
loo_cubic  <- loo(log_lik_cubic) # Warning: Some Pareto k diagnostic values are too high. See help('pareto-k-diagnostic') for details.

loo_spline <- loo(log_lik_spline)

# Compare them
loo_compare(loo_linear, loo_quad, loo_cubic) # loo_spline
```
